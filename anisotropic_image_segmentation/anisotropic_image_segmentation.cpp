...
int main()
{
    ...
    Mat imgIn = imread(p_file, IMREAD_GRAYSCALE);
    ...
    imwrite("c:\\temp\\result.jpg", 0.5 * (imgIn + imgBin));
    imwrite("c:\\temp\\Coherency.jpg", imgCoherency);
    imwrite("c:\\temp\\Orientation.jpg", imgOrientation);
    //! [main_extra]
    return 0;
}
//! [calcGST]
